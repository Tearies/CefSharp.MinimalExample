...
namespace CefSharp.MinimalExample.Common.args
{
    public static class FileHelper
    {
        ...
        #region 只允许同时创建一个文件,并将字符串写入文件，其他线程/进程需要等待

        #region 默认格式为utf-8
        /// <summary>
        /// 只允许同时创建一个文件,并将字符串写入文件，其他线程/进程需要等待。如果文件已存在则覆盖。(文件的绝对路径)
        /// </summary>
        /// <param name="filePath">文件的绝对路径</param>
        /// <param name="text">字符串数据</param>
        public static void MutexCreateFile(string filePath, string text, string mutexKey)
        {
            ThreadPool.QueueUserWorkItem(_ =>
            {
                MutexExec(mutexKey, () => { CreateFile(filePath, text, Encoding.UTF8); });
            });
        }



        private static void MutexExec(string mutexKey, Action action)
        {
            using (Mutex mut = new Mutex(initiallyOwned: false, name: mutexKey))
            {
                ...
                catch (AbandonedMutexException ex)
                {
                    ...
                    MutexExec(mutexKey: mutexKey, action: action);
                }
                finally
                {
                    mut.ReleaseMutex();
        ...
        public static void DeleteDir(string dirPath, bool recursive)
        {
            ...
            {
                throw new Exception("删除文件夹:" + dirPath, ex);
            }
        }
        #endregion
        ...
        public static string ConnectPath(char separate, params string[] path)
        {
            ...
            if (path.Length == 2) return $"{path[0].TrimEnd(separate)}{separate}{path[1].TrimStart(separate)}";
